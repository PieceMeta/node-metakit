...
class OSC extends _tinyEmitter2.default {
  ...
  listen(forBundles = true) {
    ...
    const _ctx = this,
      ...
      _ctx.emit('message', msg, timeTag && Array.isArray(timeTag.raw) ? _data.types.make(_data.types.MKT_TIMESTAMP, timeTag.raw[0] * Math.pow(2, 32) + timeTag.raw[1]) : _data.types.make(_data.types.MKT_TIMESTAMP), info);
    },
          handleBundle = (bundle, info = undefined) => {
      _ctx.emit('bundle', bundle.packets, bundle.timeTag && Array.isArray(bundle.timeTag.raw) ? _data.types.make(_data.types.MKT_TIMESTAMP, bundle.timeTag.raw[0] * Math.pow(2, 32) + bundle.timeTag.raw[1]) : _data.types.make(_data.types.MKT_TIMESTAMP), info);
    };
  ...
  sendBundle(messages, tSeconds = undefined) {
    ...
    if (typeof tSeconds === 'undefined') {
      tSeconds = _data.types.make(_data.types.MKT_TIMESTAMP);
    }
    const bundle = {
